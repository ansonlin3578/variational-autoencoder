...
# print(label[2], label[226], label[840], label[1474])
# test = data[21].cpu().detach().numpy()
# test = test[:, :, ::-1]
...
def plot_loss(losses , losses_avg):
    ...
    pic_name = "VAE_Loss_low_code_epoch{}.png".format(num_epochs)
    plt.ylabel("Loss")
...
torch.save(model.state_dict(), r"C:\Users\User\Desktop\碩一上修課資料\深度學習_林嘉文\HW2\VAE_model_weights.pth")
...
def visualize(img_batch, model):
    ...
    for i in range(batch_size):
        ...
        plt.imshow(img)
        break
    plt.savefig("VAE_test_low_code.png")
    plt.show()
...
gen_label = gen_label.flatten()
np.save(r"C:\Users\User\Desktop\碩一上修課資料\深度學習_林嘉文\HW2\VAE_gen_data.npy", gen_data)
np.save(r"C:\Users\User\Desktop\碩一上修課資料\深度學習_林嘉文\HW2\VAE_gen_label.npy", gen_label)
...
def save_img(choose_reco, choose_label, start_batch_idx , img_select , choose_idx):
    ...
    plt.title("VAE_idx={}_reconstructed".format(choose_idx), loc="center")
    plt.xlabel("VAE_label={}".format(choose_label[i][img_select-1].cpu().detach().numpy()) , loc="center")
